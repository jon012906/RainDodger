...
Assertions in Swift Testing: `#expect` as the default, `#require` for prerequisites, throwing-behavior checks, and known-issue handling. Use when writing or reviewing assertions.
...
- Use the return value to unwrap optionals safely and reduce noisy optional chaining.
- Prefer this pattern over manual optional checks when failure should halt test flow.

### Optional precondition + unwrapped usage
...
- Remove known-issue wrappers once the failure condition is fixed.

### Scope only the failing section
