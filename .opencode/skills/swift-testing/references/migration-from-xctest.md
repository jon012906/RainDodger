...
Incremental migration of existing XCTest code to Swift Testing while preserving safety and CI signal. **Future reference** — Rain Dodger has no XCTest tests yet; this file applies when migration is actually planned.

> The repo has no test target and no XCTest today. This reference is kept for when the first tests are added (decide Swift Testing directly — see `rain-dodger-conventions.md`) or when XCTest code is introduced or migrated later.
