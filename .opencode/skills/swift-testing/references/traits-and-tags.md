...
Traits, tags, availability, and test-plan filtering for controlling execution behavior and organizing large suites. Use when controlling test execution, linking bug context, or targeting runs and CI filtering.
...
- Use `@available` on tests when the entire behavior is OS-gated.
- Prefer `@available` over inline runtime checks for clearer reporting semantics.
- Never apply `@available` to suite types — only to test functions.
