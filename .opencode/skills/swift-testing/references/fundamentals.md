...
Building blocks of Swift Testing: `@Test`/`@Suite` structure, suite organization, and defaults. Use when creating new test suites or refactoring test structure.

## Building blocks

- Import `Testing` only in test targets, never in app/library targets.
...
- If a suite has instance test methods, it must have a callable zero-argument
  initializer (implicit or explicit, sync/async, throwing or non-throwing).
- If initialization requirements cannot be met, convert tests to static/global
  functions or refactor suite state.
- Suite types (and containing types) must always be available; do not apply
  `@available` to suite declarations.

### Zero-argument initializer requirement
