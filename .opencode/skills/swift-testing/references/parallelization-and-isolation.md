...
Default parallel execution model, isolation strategy, and `.serialized` as a temporary transition step. Use when tests are flaky in CI, have hidden ordering dependencies, or need to scale execution speed safely.
...
### Hidden dependency exposed by parallel execution
...
- Isolate state per test invocation (a fresh suite instance helps).
...
- **Fix shared state before reaching for `.serialized`** — it is a temporary transition step, not an architecture.
...
- If tests hit a shared DB/file/service, choose one:
  ...
  - create a separate serial test plan for the integration path
