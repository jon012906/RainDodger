...
Parameterized tests: when to use them, single/multi-argument forms, `zip` pairing and its pitfalls, and scaling strategy. Use when repeated tests share logic and differ only in input values.
...
```swift
// Before: freeFeatureA() / freeFeatureB() — near-duplicate tests.
import Testing
...
**Silent truncation**: `zip` stops at the shorter collection — extra elements are dropped with no compiler error and no test failure, just missing coverage.

```swift
// ❌ Silent gap: the last input is never tested
@Test(arguments: zip(
    [Status.active, .inactive, .pending, .banned],
    ["Active", "Inactive", "Pending"]  // one short
))
...
**Case-order fragility with `CaseIterable`**: pairing two `allCases` arrays with `zip` breaks silently if enum cases are ever reordered.
...
When inputs and expected outputs must be paired, prefer these over `zip` to avoid silent truncation and case-ordering problems.
...
- Ensure argument types are readable in output; provide a custom test description if noisy.
- Keep argument lists easy to scan (multi-line formatting is recommended).

## When `CaseIterable.allCases` is appropriate

Using `allCases` as arguments is a valid pattern for **property-based tests** — tests that verify a universal property holds for every member of a type. The key distinction: the expected result derives from the *property being tested*, not from a hard-coded mapping.
...
func fullRotation(orientation: Orientation) {
    #expect(orientation.rotated(.clockwise).rotated(.clockwise).rotated(.clockwise).rotated(.clockwise) == orientation)
}
```
...
//    this passes because rawValue has the same casing bug.
...
- **Control flow in test bodies**: `if`/`switch` inside a parameterized test body mirrors implementation logic. Tests that branch the same way as production code verify themselves rather than the behavior independently — separate the special case into its own test.
