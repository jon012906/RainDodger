...
Testing async/await code, bridging callback APIs, and confirming asynchronous events. Use when tests involve async functions, completion handlers, streams/events, or timing-related flakiness.

> Rain Dodger services are `async/await` by convention (`.opencode/rules/003-project-guideline.md`), so tests `await` service calls directly — callback bridging is the exception, not the norm. Reach for continuations only when bridging legacy completion-handler APIs.
...
- Keep non-UI tests off the main actor to preserve realistic concurrency behavior.
