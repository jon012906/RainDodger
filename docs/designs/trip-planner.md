...
- **TripPlannerSheet** — the half-modal sheet; owns the header, grouped card, Leave-at row, Check Route button, cards area, and the "Route details" row; renders `TripPlannerViewModel` state. Dismissed by drag only. The destination row is editable via destination-mode search (re-plans, saves recents); the "Route details" row (below the cards, ≥ 44 pt, chevron) expands the same sheet to `.large` with the step view (see `docs/designs/route-detail.md`).
...
- VoiceOver labels (exact):
  ...
  - Destination row: **"Destination, <name>"** (hint: "Double tap to change destination").
  ...
  - Route-summary pill: **"Route summary, X minutes, Y kilometers"** when loaded / **"Route summary, calculating route"** while routing is in flight; hint "Double tap or drag up to reopen trip planner".
