...
- User goal: after tapping Check Route, immediately see which parts of the road will be rained on at arrival time, how much of the ride is wet — with weather fetched only on Check Route (no auto-fetch when switching routes).
...
| Design §1 | Re-selected route (no auto-fetch) | Steps 5, 7 |
...
flowchart TD
  ...
  K -->|"yes · cached segments"| M["Reuse existing segments instantly · legend stays · R7"]
  ...
  N -->|"origin / stop / destination / departure · re-route only · weatherState = .idle"| P["Routing succeeds · no weather · no overlay"]
