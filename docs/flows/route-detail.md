...
- Success outcome: the rider re-taps the selected card, reads the wet-step preview, and returns to the route cards — with origin/destination/stop/selection/departure untouched and no weather ever fetched without an explicit tap.

## 3. Spec Coverage

| Spec ID | Requirement | Covered In Flow Step |
|---|---|---|
| R1 | Re-tap selected card expands same sheet (`.medium`→`.large`), back returns; state preserved | Steps 1–2, 5, 10, edges A, B |
...
| R6 | No forecast → "Check route for rain" button (`checkRoute()`); checking state; no silent auto-fetch | Steps 6–8, edge C |
...
|---|---|---|
| Design §1 | Route cards (unchanged) | Step 1 |
...
1. Rider taps "Check Route" (or auto-plans on destination pick) → routing + forecast finish; route cards render and the map shows colored rain segments (R13).
2. Rider re-taps the **already-selected** route card → the SAME planner sheet expands `.medium` → `.large` (system detent animation); the step view appears: back chevron · "Route details" · summary row · step list (R1, R2).
...
7. Rider taps "Check route for rain" → the existing `checkRoute()` runs routing + weather; the header shows "Checking rain along your route…" with an inline spinner; step marks withheld (R6).
...
- **A. Tapping a different (non-selected) card:** only selects the route (existing behavior) — no expansion; if that route has no cached segments, the existing re-fetch path runs (R1).
...
- **C. Check route for rain tapped twice:** `checkRoute()` cancels any in-flight task and restarts (existing behavior); the checking state stays until the new result or failure lands (R6).
...
- **M. Weather loading from the route-card view:** the map-level blocking overlay covers the map (existing `WeatherLoadingOverlay`); the step view's own checking state only shows when the rider is already in the step view (R6).
...
```mermaid
flowchart TD
  A["Planner sheet open · route cards · selected route + forecast · R13"] --> B{"Rider taps the ALREADY-selected card? · R1"}
  B -->|"no (different card)"| C["Select route only · existing re-fetch path · edge A"]
  ...
  H2 -->|"yes"| I["checkRoute() · checking rain state in header · R6"]
  H2 -->|"no"| BACK
