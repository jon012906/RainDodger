...
Give the rider a glanceable sense of which way they face and direct control over map orientation while mounted: a gyro-driven **head arrow** at their position on the map (replacing the system `UserAnnotation` blue dot), the **always-visible custom gyro compass** (restored — the built-in `MapCompass` was rejected because it reflects map orientation and hides at north-up), and a **heading-lock button** that rotates the map to follow the phone's heading. One glance answers "which way am I facing" and "is the map locked to my heading"; one tap locks/unlocks the map to the heading or recenters.
...
- Apple Maps riders expect a compass; the platform one only appears off-north and shows map orientation, which does not answer "which way is the phone heading" while the map is north-up. A custom always-visible dial carrying the phone heading answers that.
- The built-in `MapCompass` reflects map orientation and hides when the map is north-up; the rider's expectation is an always-visible compass that follows the **phone's** heading, so the app ships a custom gyro compass (`.userLocation(followsHeading:)` is the native camera mode it pairs with).
...
| NH4 | Custom gyro compass, always visible, top of bottom-trailing control stack | P0 | 56 pt white circle + gray ring + red needle rotating with `viewModel.heading` (0 when nil), fixed N, decorative a11y-hidden, NOT tappable |
...
| NH7 | Lock-state sync from the real camera | P0 | `.onChange(of: cameraPosition)` → `syncFollowHeading(newPosition.followsUserHeading)`; user pan and destination focus keep the button in sync — no camera echo |
...
- **Lock-state sync (no camera echo):** the button never writes the camera; `.onChange(of: cameraPosition)` mirrors the real camera's `followsUserHeading` into `isHeadingLocked`, so user pans and destination focus stay honest.
...
- No native `MapCompass` (it reflects map orientation and auto-hides north-up, so it cannot meet the always-visible expectation), no compass tap behavior (reorient happens via `RecenterButton`/lock), no `MapUserLocationButton` (its double-tap heading behavior would fight the lock button).
...
- [ ] NH4: custom gyro compass at the top of the bottom-trailing control stack (above `RecenterButton`) — always visible; 56 pt white circle + gray ring + red needle rotating with `viewModel.heading` (0 when nil → upright in sim); fixed N; decorative a11y-hidden; not tappable
...
- [ ] NH7: lock-state synced from the real camera (`followsUserHeading`) — pan, destination focus keep the button honest; no camera echo
