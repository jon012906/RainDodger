...
|---|---|---|---|
| R1 | "Direction" half-modal sheet auto-opens immediately when a destination is picked in the search page (pick = confirm); the trip rows sit in a grouped card; it dismisses by dragging down (drag indicator, **no generic close button**), or via the destination-row X that clears the destination and dismisses the sheet (R18); dismissing keeps everything on the map as-is while the route-summary pill (R13) floats over it — tapping the pill reopens the sheet with all state preserved | P0 | Detents medium/large, drag indicator visible (drag-to-dismiss); dismissed sheet → map unchanged (route alternatives, selection, pins, camera) + route-summary pill bottom-center |
...
| R3 | Destination row is editable: tap opens the reused `SearchPage` in destination mode; picking a new destination calls `updateDestination` (re-plans the route) and saves the pick to recents (same `SearchContext.destination.savesRecents` behavior as the map search flow); the row shows the category icon (orange) + the confirmed destination name | P0 | Re-plan on pick; recents consistent with the map search flow; icon reuses `SearchResult.categorySymbol` |
...
| R9 | Edge cases handled gracefully | P0 | See flow doc edge cases A–W |
...
| R13 | Route-summary pill: a long Apple Maps–style pill with a grabber, shown while the sheet is dismissed and routing has not failed; shows the selected route's ETA·distance once a plan exists, "Calculating…" while a request is in flight; tapping OR dragging the pill up (≥ 40 pt) reopens the sheet preserving origin/destination/stop/selection | P0 | Long pill with a decorative grabber bar; drag-up ≥ 40 pt reopens (tap retained as the accessible path); floats bottom-center above the safe area, ≥ 44 pt; see Rules/Logic + flow edge cases J–K |
...
| R15 | "Leave at" row with a gray pill — date-aware text: "Now" (no departure) · same-day time "2:00 PM" · any other day "27 Sep 2:00 PM" (day + abbreviated month + time): tappable → `DepartureTimePickerSheet` (Apple Maps "Leave at" style, `.large` sheet): header X (cancel, no re-plan) · "Leave at" · blue up-arrow (apply); `.graphical` date-only calendar (min = today, past days disabled); "Time" row + capsule pill opening a nested `.height(300)` wheel hour+minute sub-sheet (blue "Done" commits the wheel draft into the picker draft); "Leave Now" row. Draft-commit: date/time changes are picker-local drafts; re-route happens ONLY on arrow-apply (`setDepartureDate(draftDate)`) or "Leave Now" (`setDepartureDate(nil)`); X/swipe-dismiss cancels with no re-plan. Applied departure runs routing with `MKDirectionsRequest.departureDate` (traffic-aware ETA) | P0 | VM `setDepartureDate` stays the single commit point (clamps past → now); "Leave now" resets to Now; draft default = `departureDate ?? Date()` (now's time, time preserved on date change); pill shows the picked date so a different-day selection is always visible; no weather re-scoring on this branch (Phase 4) |
| R16 | "Check Route" button: full-width blue, motorcycle icon, ≥ 44 pt; dismisses the trip sheet first, then runs `checkRoute()` (idempotent refresh that doubles as Retry); on routing failure while the sheet is dismissed, `MapScreenView` re-presents the trip sheet so the error + Retry (Check Route) stay reachable | P0 | Auto-plan on destination pick unchanged; tapping while loading cancels + restarts the plan |
| R17 | Clear from the route-summary pill: the pill carries a trailing X (≥ 44 pt) shown whenever the pill is shown; tapping it clears the destination immediately, with no confirmation | P0 | Reset scope per §5 "Clear destination"; empty state per §5; accepted limitation: while weather is loading the blocking overlay covers the pill, so the pill's X is unreachable until the fetch ends |
| R18 | Clear from the destination row: the destination row carries a trailing X (≥ 44 pt) shown only when a destination is set; tapping it clears the destination and dismisses the trip sheet, landing on the same empty map state as R17 | P0 | Same reset scope as R17; sheet dismissal is the system drag-down dismiss; camera returns to the rider's location |
...
- **Destination edit:** the destination row is tappable and opens the reused `SearchPage` in destination mode; picking a new destination calls `updateDestination` (clears the plan, re-runs routing) and saves the pick to recents via the existing `SearchContext.destination.savesRecents` behavior — consistent with the map search flow. The row is no longer read-only.
...
- **Failure:** routing error/offline → `failed` state with a Retry action. When the failure lands while the trip sheet is dismissed (e.g. a Check Route that dismissed the sheet first), `MapScreenView` re-presents the trip sheet so the error message and Retry (the Check Route button) stay reachable.
- **Pill visibility:** the route-summary pill shows while the sheet is dismissed and routing has not failed; it is a long Apple Maps–style pill with a grabber bar; it shows the selected route's ETA·distance once a plan exists and "Calculating…" while a request is in flight; the pill is hidden while the sheet is open or when routing failed.
...
- **Reopen preserves state:** tapping the pill OR dragging it up (upward translation ≥ 40 pt) reopens the half-sheet with all state intact (origin, destination, stop, selected route, alternatives) and does not re-run routing; a re-route happens only after an origin/stop/destination change inside the reopened sheet.
...
- **Check Route button:** full-width blue button; tapping it dismisses the trip sheet first, then runs `checkRoute()` — idempotent (same trip → same plan) and doubles as Retry after a failure; tapping while a request is in flight cancels and restarts. Auto-plan on destination pick is unchanged. If routing fails after the sheet was dismissed, `MapScreenView` re-presents the trip sheet so the error + Retry are reachable.
- **Clear destination:** both the route-summary pill's trailing X (R17) and the destination row's trailing X (R18) reset the trip to the empty search state. The reset clears the destination ONLY: `destination = nil`, `routePlan = nil`, `selectedRouteID = nil`, `state = .idle`, `weatherState = .idle`, cancel any in-flight plan/weather tasks, and clear `MapViewModel.selectedDestination` (so no destination pin remains). Origin, stop, and `departureDate` are RETAINED for the next plan. The camera returns to the rider's location. The empty end state shows the `DestinationSearchField` ("Your Destination…") with no destination pin, no route polyline, and no rain overlays/legend/badges. Picking a new destination afterwards plans normally with the retained origin/stop/departure; a custom origin is preserved (the map search refreshes origin only when it is unset or still the auto "Current location"). The action is idempotent — rapid double-taps are safe (the second tap is a no-op on already-cleared state). Accepted limitation: while weather is loading the blocking overlay sits above the pill, so the pill's X is unreachable until the fetch ends.
- **Route details entry:** a "Route details" row below the route cards is the clear, discoverable entry to the step-by-step view (see `docs/specs/route-detail.md`); re-tapping the already-selected route card remains a shortcut to the same view.
...
- [ ] `docs/template/spec-guide.md` §9 shows `fix/clear-destination` with trip-planner Models/Services/ViewModels/Views
...
- [ ] Origin row editable through the existing `SearchPage` in "where from?" mode; destination row editable through the reused `SearchPage` in destination mode — picking re-plans and saves to recents (R3)
...
- [ ] Sheet dismisses by dragging down (no generic close button) or via the destination-row X that clears and dismisses (R18); the map keeps the route/selection/pins/camera as-is and the long route-summary pill (grabber bar) shows the selected route's ETA + distance ("Calculating…" while loading); tapping it OR dragging it up (≥ 40 pt) reopens the sheet with origin/destination/stop/selection preserved (R13)
...
- [ ] "Leave at" row shows a gray date-aware pill ("Now" · same-day "2:00 PM" · any other day "27 Sep 2:00 PM"); picking a different date is always visible in the pill; tapping opens the Apple Maps–style `DepartureTimePickerSheet` (`.large` sheet: X · "Leave at" · blue up-arrow header, `.graphical` date-only calendar min = today, Time row + nested wheel sub-sheet with Done, Leave Now); draft-commit — re-plan only on arrow-apply or "Leave Now", X/swipe cancels without a re-plan; applied departure re-runs routing with `MKDirectionsRequest.departureDate` for traffic-aware ETA; past dates disabled and a past applied time clamped to now; no weather re-scoring (R15)
- [ ] "Check Route" button (full-width blue, motorcycle icon, ≥ 44 pt) dismisses the trip sheet first, then runs `checkRoute()` idempotently; doubles as Retry; tapping while loading cancels + restarts; on routing failure while dismissed the map re-presents the trip sheet so the error + Retry stay reachable; auto-plan on destination pick unchanged (R16)
- [ ] Route-summary pill carries a trailing X (≥ 44 pt) shown whenever the pill is shown; tapping it clears the destination immediately (no confirmation), hides the pill, and returns the map to the empty search state (R17)
- [ ] Trip planner destination row carries a trailing X (≥ 44 pt) shown only when a destination is set; tapping it clears the destination and dismisses the sheet to the same empty map state (R18)
- [ ] Clearing removes the destination pin, route alternatives/selection, rain overlays/legend/badges, and resets `state` + `weatherState` to `.idle`, cancels in-flight plan/weather tasks, and clears `MapViewModel.selectedDestination`; origin/stop/departure are retained; the camera returns to the rider's location; re-picking plans normally with the retained values (R17, R18)
- [ ] A "Route details" row below the route cards is the clear entry to the step view; re-tapping the already-selected card remains a shortcut (`docs/specs/route-detail.md` R1)
- [ ] Edge cases A–W per `docs/flows/trip-planner.md`
